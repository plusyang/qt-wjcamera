...
static void fill1(char *a, int n, int width, int height, int stride)
{
    int i, j;
    /* paint the buffer with colored tiles */
    for (j = 0; j < height; j++) {
        uint32_t *fb_ptr = (uint32_t*)((char*)a + j * stride);
        for (i = 0; i < width; i++) {
...
//    CHECK_GL(glEnableVertexAttribArray(_filterPositionAttribute));
//    imageData = new GLubyte[1920*1080*4];
//    memset(imageData, 0, 1920*1080*4);
...
//                EGL_NONE
//        };
//      PFNEGLCREATEIMAGEKHRPROC _eglCreateImageKHR = (PFNEGLCREATEIMAGEKHRPROC) eglGetProcAddress("eglCreateImageKHR");
//      fill1((char*)map, 0, w, h, stride);
//      return _eglCreateImageKHR(eglGetCurrentDisplay(), EGL_NO_CONTEXT,
...
//    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, 1920, 1080, 0, PIXEL_FORMAT, GL_UNSIGNED_BYTE, (GLvoid*)imageData);
//    glBindTexture(GL_TEXTURE_2D, 0);
    //glEnableVertexAttribArray(_filterTexCoordAttribute);
//    int width = Context::getInstance()->captureWidth;
//    int hight = Context::getInstance()->captureHeight;


    glBindBuffer(GL_PIXEL_PACK_BUFFER, pboIdes[0]);
    glBufferData(GL_PIXEL_PACK_BUFFER, 1920*1080*4, nullptr, GL_STREAM_DRAW);
    glBindBuffer(GL_PIXEL_PACK_BUFFER, pboIdes[1]);
    glBufferData(GL_PIXEL_PACK_BUFFER, 1920*1080*4, nullptr, GL_STREAM_DRAW);
    Context::getInstance()->capturedFrameData = new unsigned char[1920*1080*4];
...
void Filter::update(float frameTime) {
    ...
    if (Context::getInstance()->isCapturingFrame && this == Context::getInstance()->captureUpToFilter) {
        ...
        //printf("captureWidth  =%d, captureHeight = %d",captureWidth,captureHeight);
        static int index = 0;
        int nextIndex = 0;
        index = (index + 1) % 2;
       // qDebug() << "index:" << index;
        nextIndex = (index + 1) % 2;
        _framebuffer = Context::getInstance()->getFramebufferCache()->fetchFramebuffer(this->_contexID, captureWidth, captureHeight);
        proceed(false);

        _framebuffer->active();
        glBindBuffer(GL_PIXEL_PACK_BUFFER, pboIdes[index]);
        glReadPixels(0, 0, captureWidth, captureHeight, GL_RGBA, GL_UNSIGNED_BYTE, 0);
        //printf("********************%d\n",pboIdes[nextIndex]);
        CHECK_GL(glBindBuffer(GL_PIXEL_PACK_BUFFER, pboIdes[nextIndex]));
        Context::getInstance()->capturedFrameData = (unsigned char *)glMapBufferRange(GL_PIXEL_PACK_BUFFER, 0, captureWidth*captureHeight*4, GL_MAP_READ_BIT);
       // qDebug()<<Context::getInstance()->capturedFrameData[captureWidth*captureHeight]<<Context::getInstance()->capturedFrameData[captureWidth*captureHeight+1]<<Context::getInstance()->capturedFrameData[captureWidth*captureHeight+2];
        ...
        _framebuffer = Context::getInstance()->getFramebufferCache()->fetchFramebuffer(this->_contexID, rotatedFramebufferWidth, rotatedFramebufferHeight);
//        qDebug()<<"create framebuffer textureID: "<<_framebuffer->getTexture();
//        qDebug()<<"create framebufferID: "<<_framebuffer->getFramebuffer();
        proceed();
    }
