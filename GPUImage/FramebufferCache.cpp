...
Framebuffer* FramebufferCache::fetchFramebuffer(int contexID, int width, int height, bool onlyTexture/* = false*/, const TextureAttributes textureAttributes/* = defaultTextureAttribure*/) {
    
    Framebuffer* framebufferFromCache = 0;
    std::string lookupHash = _getHash(contexID, width, height, onlyTexture, textureAttributes);
    ...
    if (numberOfMatchingFramebuffers < 1) {
        framebufferFromCache = new Framebuffer(contexID, width, height, onlyTexture, textureAttributes);
    } else {
        ...
        while (!framebufferFromCache && curFramebufferId >= 0) {
            ...
            // qDebug()<<"--fetchFramebuffer--------framebufferHash:    "<<framebufferHash.c_str();
...
void FramebufferCache::returnFramebuffer(Framebuffer* framebuffer) {
    ...
    int contexID = framebuffer->getContexID();
    const TextureAttributes& textureAttributes = framebuffer->getTextureAttributes();
    std::string lookupHash = _getHash(contexID, width, height, !framebuffer->hasFramebuffer(), textureAttributes);
    ...
    // qDebug()<<"--returnFramebuffer--------framebufferHash:    "<<framebufferHash.c_str();
    _framebuffers[framebufferHash] = framebuffer;
    _framebufferTypeCounts[lookupHash] = numberOfMatchingFramebuffers + 1;
}

std::string FramebufferCache::_getHash(int contexID, int width, int height, bool onlyTexture, const TextureAttributes textureAttributes) const {
    if (onlyTexture)
    {   std::string str = str_format("[%d]: %.1dx%.1d-%d:%d:%d:%d:%d:%d:%d-NOFB",contexID, width, height, textureAttributes.minFilter, textureAttributes.magFilter, textureAttributes.wrapS, textureAttributes.wrapT, textureAttributes.internalFormat, textureAttributes.format, textureAttributes.type);
        // qDebug()<<"----------_getHash:    "<<str.c_str();
        return str;
    }
    else
    {
        std::string str = str_format("[%d]: %.1dx%.1d-%d:%d:%d:%d:%d:%d:%d",contexID, width, height, textureAttributes.minFilter, textureAttributes.magFilter, textureAttributes.wrapS, textureAttributes.wrapT, textureAttributes.internalFormat, textureAttributes.format, textureAttributes.type);
        // qDebug()<<"----------_getHash:    "<<str.c_str();
        return str;
    }
}
