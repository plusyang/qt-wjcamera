...
SourceCamera* SourceCamera::create(int contexID) {
    SourceCamera* sourceCamera = new SourceCamera();
    sourceCamera->_contexID = contexID;
#if PLATFORM == PLATFORM_IOS
...
void SourceCamera::setFrameData(int width, int height, const void* pixels, RotationMode outputRotation/* = RotationMode::NoRotation*/) {
    ...
    Framebuffer* framebuffer = Context::getInstance()->getFramebufferCache()->fetchFramebuffer(this->_contexID, width, height, true);
